...
class GaussianExtractor(object):
    ...
    def export_image(self, path):
        ...
        normal_depth_path = os.path.join(path, "normal_depth")
        ...
        for idx, viewpoint_cam in tqdm(enumerate(self.viewpoint_stack), desc="export images"):
            ...
            t5 = time.time()
            save_img_u8(self.depth_normals[idx].permute(1,2,0).cpu().numpy() * 0.5 + 0.5, os.path.join(normal_depth_path, 'depth_normal_{0:05d}'.format(idx) + ".png"))
            t6 = time.time()
            print(f"Index {idx:05d}:")
            print(f"  GT Image Save Time: {t2 - t1:.4f} seconds")
            print(f"  RGB Map Save Time: {t3 - t2:.4f} seconds")
            print(f"  Depth Map Save Time: {t4 - t3:.4f} seconds")
            print(f"  Normal Map Save Time: {t5 - t4:.4f} seconds")
            print(f"  Depth Normal Map Save Time: {t6 - t5:.4f} seconds")
            print("")