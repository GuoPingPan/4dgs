...
def generate_path_fixed_time(viewpoint_cameras, n_frames=480, time_step = 10. / 2000):
    ...
    for i, c2w in enumerate(new_poses):
        ...
        cam.full_proj_transform = (cam.world_view_transform.unsqueeze(
            0).bmm(cam.projection_matrix.unsqueeze(0).cuda())).squeeze(0)
        cam.camera_center = cam.world_view_transform.inverse()[3, :3]
        cam.timestamp = time_step
...
def generate_path(viewpoint_cameras, n_frames=480, time_step = 10. / 2000):
    c2ws = np.array([np.linalg.inv(np.asarray(
        (cam.world_view_transform.T).cpu().numpy())) for cam in viewpoint_cameras])
    pose = c2ws[:, :3, :] @ np.diag([1, -1, -1, 1])
    pose_recenter, colmap_to_world_transform = transform_poses_pca(pose)
...
def create_video_from_images(input_dir, output_name, fps):
    # 输入文件夹
    render_folder = os.path.join(input_dir, 'renders')
    depth_folder = os.path.join(input_dir, 'depth')
    
    # 获取文件夹内的所有图片文件
    render_images = sorted([os.path.join(render_folder, f) for f in os.listdir(render_folder) if f.endswith(('.png', '.jpg', '.jpeg'))])
    depth_images = sorted([os.path.join(depth_folder, f) for f in os.listdir(depth_folder) if f.endswith(('.png', '.jpg', '.jpeg'))])
    
    # 检查文件夹是否为空
    if not render_images:
        raise ValueError("Render folder is empty or does not contain valid image files.")
    if not depth_images:
        raise ValueError("Depth folder is empty or does not contain valid image files.")
    
    # 使用第一张图片来获取视频的宽高
    first_render_image = cv2.imread(render_images[0])
    height, width, _ = first_render_image.shape

    # 创建视频写入对象
    render_video_path = f"{output_name}_render.mp4"
    depth_video_path = f"{output_name}_depth.mp4"
    
    render_video_writer = cv2.VideoWriter(render_video_path, cv2.VideoWriter_fourcc(*'mp4v'), fps, (width, height))
    depth_video_writer = cv2.VideoWriter(depth_video_path, cv2.VideoWriter_fourcc(*'mp4v'), fps, (width, height))

    # 写入render视频
    for image_path in render_images:
        img = cv2.imread(image_path)
        render_video_writer.write(img)
    
    # 写入depth视频
    for image_path in depth_images:
        img = cv2.imread(image_path)
        depth_video_writer.write(img)
    
    # 释放资源
    render_video_writer.release()
    depth_video_writer.release()
    
    print(f"Videos saved as {render_video_path} and {depth_video_path}")

def create_video_from_images_h264(input_dir, output_name, fps):
    # 输入文件夹
    render_folder = os.path.join(input_dir, 'renders')
    depth_folder = os.path.join(input_dir, 'depth')
    ...
    render_video_writer = cv2.VideoWriter(render_video_path, fourcc, fps, (width, height))
    depth_video_writer = cv2.VideoWriter(depth_video_path, fourcc, fps, (width, height))
